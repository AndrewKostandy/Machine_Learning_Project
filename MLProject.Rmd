...
c5Grid<-expand.grid(trials=20, model="tree", winnow=TRUE)

c5modFitFinal<-train(classe~.,data=training4,method="C5.0",verbose=FALSE, tuneGrid=c5Grid)
```
...
Indeed, the accuracy of our model on our testing data set comes out to be almost exactly equal to what we expected, coming at **0.9929**. Therefore, we are now also more confident about our expected out of sample error rate.
...
We'll continue to use the best tuning parameters we got when training the c5modFit model.
